...
- `src/aqua_qe_product_manager/services/` — integrações externas: `llm_service` (Ollama), `embedding_service` (Ollama, `bge-m3`) + `rag_service` (Qdrant embarcado — memória institucional de refinamento, ver convenção abaixo), `jira_service` (Jira Cloud REST API, **apenas leitura**), `confluence_service` (Confluence Cloud REST API — leitura, criação de página nova e atualização de página existente, usado por `--publicar-confluence`/`--atualizar-confluence`).
...
- **Piloto de provedor via toggle** (`LLM_PROVIDER=ollama|nvidia`, padrão `ollama`): `llm_service.generator_model()`/`reviewer_model()` resolvem o modelo certo conforme o provedor ativo; `complete`/`complete_json` mantêm assinatura inalterada e despacham internamente para Ollama ou para NVIDIA NIM (`build.nvidia.com`, API compatível com OpenAI via pacote `openai`). Este agente é o piloto escolhido entre os três irmãos (PM/PO/SA) para essa avaliação — Ollama continua o padrão sem `LLM_PROVIDER` definido, e nenhuma skill geradora precisa saber do provedor (só `complete`/`complete_json`). Embedding continua sempre Ollama (`embedding_service.py`, `bge-m3`), sem toggle — não afetado por `LLM_PROVIDER`.
...
- **`refine_prd_draft` (workflow) re-deriva seletivamente os 10 campos de profundidade depois de um refinamento**, via `_DEPENDENCIAS_PROFUNDIDADE` (mapa skill → campos centrais dos quais depende, em `workflow/generate_prd.py`): só re-executa a skill de um campo de profundidade se o diff entre o draft antes/depois de `refine_prd` mostrar que um campo central mapeado para ela realmente mudou. O mapa é heurístico (a maioria das skills recebe o texto inteiro do PRD, não um campo isolado) e deliberadamente enviesado para re-executar na dúvida — nunca pular por engano —, preservando o cuidado aprendido com um bug real de obsolescência já corrigido no agente irmão AQuA-QE Solution Architect (ciclo de refinamento que não revalidava). A geração inicial (`generate_prd_draft`) não passa o diff, então sempre roda as 10 skills, sem exceção. `refine_prd_draft(..., forcar_rederivacao_completa=True)` ignora o mapa e volta ao comportamento de sempre re-derivar tudo, como válvula de escape.
- **Sem RAG sobre `knowledge/methodology/` nesta fase** — pequeno o suficiente para caber direto no prompt de cada skill (ver `docs/agent/context_engineering.md`). Ideia distinta, ainda sem consumidor real.
- **Memória institucional de respostas de refinamento** (`record_refinement_answer`/`suggest_refinement_answer`, `rag_service.py`, issue [#9](https://github.com/dufelizardo/AQuA-QE-Product-Manager/issues/9)): cada resposta que o humano dá num ciclo de refinamento (visão, estratégia ou PRD) é gravada numa collection Qdrant embarcada própria (`refinement_answer_memory`) via embedding local (`bge-m3`). No ciclo seguinte, se uma pergunta parecida aparecer (mesmo ou outro artefato/projeto), a resposta mais similar já dada é exibida como sugestão no terminal, com o score de similaridade — **nunca aplicada automaticamente**: o humano sempre digita a resposta no `input()` normalmente. Sem gate de score mínimo (sem corpus histórico para calibrar um threshold) e sem filtro por tipo de artefato (visão/estratégia/PRD compartilham a mesma memória). Esta é a primeira infraestrutura de embedding/vetor deste agente — construída especificamente para este consumidor, não antecipadamente (mesmo princípio já aplicado no Product Owner).
