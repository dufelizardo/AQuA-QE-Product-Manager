...
- **Piloto de provedor via toggle** (`LLM_PROVIDER=ollama|nvidia`, padrão `ollama`): `llm_service.generator_model()`/`reviewer_model()` resolvem o modelo certo conforme o provedor ativo; `complete`/`complete_json` mantêm assinatura inalterada e despacham internamente para Ollama ou para NVIDIA NIM (`build.nvidia.com`, API compatível com OpenAI via pacote `openai`). Este agente é o piloto escolhido entre os três irmãos (PM/PO/SA) para essa avaliação — Ollama continua o padrão sem `LLM_PROVIDER` definido, e nenhuma skill geradora precisa saber do provedor (só `complete`/`complete_json`). Embedding está fora de escopo deste piloto (este agente não tem infraestrutura de embedding).
