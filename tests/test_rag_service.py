...
from aqua_qe_product_manager.services import rag_service
...
def test_record_refinement_answer_cria_colecao_e_permite_busca(rag_isolado):
    rag_service.record_refinement_answer(
        "Qual a postura sobre LGPD?", "Anonimizar dados sensíveis.", "prd"
    )
    ...
    assert resultados[0]["tipo_artefato"] == "prd"
...
def test_find_similar_refinement_answers_distingue_perguntas_diferentes(rag_isolado):
    rag_service.record_refinement_answer(
        "Qual a postura sobre LGPD?", "resposta sobre lgpd", "visao"
    )
    rag_service.record_refinement_answer(
        "Qual a estratégia de rollout?", "resposta sobre rollout", "estrategia"
    )
