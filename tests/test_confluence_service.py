...
from aqua_qe_product_manager.services import confluence_service
...
def test_texto_para_storage_converte_negrito_markdown():
    resultado = confluence_service._texto_para_storage("- **Termo**: definicao do termo")

    assert resultado == "<ul><li><strong>Termo</strong>: definicao do termo</li></ul>"
...
def test_texto_para_storage_converte_tabela_markdown():
    texto = (
        "## Objetivos de Negócio (KPI)\n\n"
        "| Objetivo | KPI |\n"
        "|---|---|\n"
        "| Reduzir fila | 30% |\n"
        "| Aumentar satisfacao | NPS |\n\n"
    ...
    assert resultado == (
        "<h2>Objetivos de Negócio (KPI)</h2>"
        "<table><tbody>"
        "<tr><th>Objetivo</th><th>KPI</th></tr>"
        "<tr><td>Reduzir fila</td><td>30%</td></tr>"
        "<tr><td>Aumentar satisfacao</td><td>NPS</td></tr>"
        "</tbody></table>"
