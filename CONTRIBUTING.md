...
Obrigado por considerar contribuir com o **AQuA-QE Product Manager**! Antes de mais nada, vale a pena ler o `WHITEPAPER.md` (ou `WHITEPAPER.en.md`) e `docs/agent/` para entender o que o agente faz e por quê — a maioria das dúvidas sobre "por que isso não é feito assim" já está respondida lá.

## Relatando problemas

- Confira as [issues existentes](https://github.com/dufelizardo/AQuA-QE-Product-Manager/issues) antes de abrir uma nova.
- Se for algo que parece uma lacuna conhecida (ex.: uma integração ou capacidade ainda não implementada), veja primeiro o [Project "Backlog"](https://github.com/users/dufelizardo/projects/4) — pode já estar lá, deliberadamente adiado até haver um consumidor real.
- Ao relatar um bug, inclua: passos para reproduzir, comportamento esperado vs. observado, modo usado (`--modo descoberta/visao/estrategia/prd/completo`), e o provedor de LLM ativo (`LLM_PROVIDER`, se diferente do padrão `ollama`).
...
- Mudanças que permitam a uma skill inventar dado fora da fonte de entrada, ou que contornem a revisão humana, são rejeitadas. O guardrail mais crítico deste agente é **GR-M1** (nunca listar concorrente/tendência de mercado que não esteja literalmente citado no texto de entrada, mesmo que o modelo "reconheça" o mercado descrito) — ver `docs/agent/guardrails.md` para o conjunto completo (GR-M1/GR-M2/GR-M3 cobrem mercado, métricas financeiras e metas de visão/estratégia).
...
- [Wiki](https://github.com/dufelizardo/AQuA-QE-Product-Manager/wiki) — visão geral com links para tudo.
- [Discussions](https://github.com/dufelizardo/AQuA-QE-Product-Manager/discussions) — comece pelo post "Welcome".
- [Backlog project](https://github.com/users/dufelizardo/projects/4) — o que está deliberadamente fora desta fase.
