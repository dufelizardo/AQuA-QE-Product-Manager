...
Ainda dentro do PRD, logo após `generate_prd` produzir os 9 campos centrais, um conjunto de 9 skills (`identify_user_journeys`, `identify_business_objectives`, `identify_use_cases`, `identify_external_dependencies`, `identify_technical_assumptions`, `identify_constraints`, `identify_prd_glossary`, `identify_candidate_product_metrics`, `identify_mvp_scope`) enriquece o rascunho com personas (reaproveitadas da descoberta quando existirem, senão via `synthesize_personas`), jornadas do usuário, objetivos de negócio com KPI, casos de uso, dependências externas, premissas técnicas, restrições, glossário de domínio, métricas de produto candidatas e agrupamento MVP vs. versão futura — sob a mesma disciplina GR-1, com uma única exceção deliberada (`identify_candidate_product_metrics`, GR-M5: sugere métricas de mercado típicas, sempre rotuladas como sugestão, nunca como fato). Depois de um refinamento (`--refinar`), esses 10 campos de profundidade **não são mais todos re-derivados incondicionalmente** — só os campos cujo mapa de dependência (`_DEPENDENCIAS_PROFUNDIDADE` em `workflow/generate_prd.py`) indica que foram afetados pelo que de fato mudou nos 9 campos centrais são re-executados; os demais preservam o valor já existente. O mapa é heurístico e deliberadamente enviesado a re-executar na dúvida, para não reintroduzir a obsolescência que motivava re-derivar tudo sempre. `refine_prd_draft(..., forcar_rederivacao_completa=True)` continua disponível como válvula de escape para o comportamento antigo (sempre re-derivar os 10). Detalhe completo em `../agent/prd.md`, `../agent/skills.md` (seção "Re-derivação seletiva no ciclo de refinamento") e `../../WHITEPAPER.md`, seção 5.1; sem diagrama próprio.
