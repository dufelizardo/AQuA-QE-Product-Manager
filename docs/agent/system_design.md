...
- **Piloto de provedor alternativo via toggle** (`LLM_PROVIDER=ollama|nvidia`, padrão `ollama`): este agente foi escolhido como piloto para avaliar NVIDIA NIM (`build.nvidia.com`, API compatível com OpenAI) como provedor opcional de gerador/revisor, mantendo Ollama como padrão inalterado. Decisão consciente de reverter, apenas para o piloto, o princípio "modelos locais via Ollama" descrito no `WHITEPAPER.md` — não uma substituição definitiva. Embedding fica fora deste piloto — continua sempre Ollama, sem toggle, mesmo com `LLM_PROVIDER=nvidia`.
- Sem RAG sobre `knowledge/methodology/` nesta fase — pequeno o suficiente para caber direto no prompt; `retrieve_chunks` (nos moldes do Product Owner) fica para uma fase futura, se o volume de conhecimento crescer. Ideia distinta da memória institucional de refinamento abaixo, ainda sem consumidor real.
- **Memória institucional de respostas de refinamento** (issue [#9](https://github.com/dufelizardo/AQuA-QE-Product-Manager/issues/9)): primeira infraestrutura de embedding/vetor deste agente — `services/embedding_service.py` (Ollama `bge-m3`) + `services/rag_service.py` (Qdrant embarcado, collection `refinement_answer_memory`, único propósito hoje — não generaliza para `knowledge/methodology/`, que continua sem consumidor). `record_refinement_answer`/`suggest_refinement_answer` gravam e sugerem — nunca aplicam automaticamente — respostas humanas dos três ciclos de refinamento (visão/estratégia/PRD). Portado do agente irmão AQuA-QE Product Owner (issue [PO#12](https://github.com/dufelizardo/AQuA-QE-Product-Owner/issues/12)), onde a mesma infra já existia para outro propósito; aqui construída do zero, especificamente para este consumidor.
