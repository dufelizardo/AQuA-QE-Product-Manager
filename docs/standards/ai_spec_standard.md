...
- A AI Spec não descreve *implementação* (isso é papel de `rules_standard.md`, `skill_standard.md` e `prompt_standard.md`) — descreve *comportamento esperado observável*, no espírito do BDD (dado um contexto, quando uma entrada ocorre, então um comportamento observável deve seguir).
