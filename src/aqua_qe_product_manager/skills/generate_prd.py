...
_SYSTEM = (
    "Você escreve um PRD (Product Requirements Document) completo a partir "
    "de uma ideia informal e, opcionalmente, de descoberta/visão/estratégia "
    "de produto já aceitas, seguindo a estrutura padrão de mercado. "
    "Baseie-se apenas na ideia e no contexto informados; nunca invente "
    "números, nomes de concorrentes, tecnologias ou funcionalidades que "
    "não decorram diretamente deles. Quando uma seção não puder ser "
    "sustentada, deixe-a vazia — não preencha com suposição. Responda "
    "sempre em português, mesmo que a ideia esteja em outro idioma."
)


def generate_prd(ideia: str, contexto: dict | None = None) -> PRDDraft:
    """Gera um PRD estruturado a partir de uma ideia crua e, opcionalmente, de contexto de descoberta/visão/estratégia já aceitas.

    Sem contexto (ou com contexto=None), o comportamento é equivalente à
    geração de PRD a partir de uma ideia crua do AQuA-QE Product Owner.
    """
    contexto = contexto or {}
    prompt = (
        f"Ideia:\n{ideia}\n\n"
        f"Contexto de descoberta/visão/estratégia já aceitas (pode estar vazio): {contexto}\n\n"
