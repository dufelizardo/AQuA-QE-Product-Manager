...
_SYSTEM = (
    ...
    "Baseie-se apenas no PRD atual e nas respostas fornecidas; nunca "
    "invente requisito, risco ou critério que não tenha sido informado "
    "neles. Nunca remova ou resuma um detalhe que já existe em um campo "
    "atual, a menos que uma resposta contradiga esse detalhe "
    "especificamente — preserve o texto existente nos campos que as "
    "respostas não abordam. Responda sempre em português, mesmo que o PRD "
    "atual contenha trechos em outro idioma."
)


def refine_prd(draft: PRDDraft, respostas: list[dict]) -> PRDDraft:
    """Reescreve os campos do PRD usando as respostas às perguntas de esclarecimento, preservando o que não foi perguntado."""
    perguntas_respostas = [
    ...
    prompt = (
        ...
        "resolvendo as lacunas apontadas. Campos (ou itens de lista) que "
        "não têm relação com nenhuma das respostas acima devem manter o "
        "texto atual, com o mesmo nível de detalhe — nunca simplifique um "
        "item para menos palavras do que já tinha.\n\n"
    ...
    draft.functional_requirements = (
        dados.get("requisitos_funcionais") or draft.functional_requirements
    )
